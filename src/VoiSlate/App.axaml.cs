...
public partial class App : Application
{
    ...
    /// <summary>
    /// Android 主视图：与 Views/MainWindow.axaml 同构的导航壳（左栏导航 + 右侧
    /// ContentControl ↔ MainViewModel.CurrentPage）。页导航数据取自 MainViewModel.Pages
    /// （单一事实源），页面本身由 App.axaml 的 VM→View DataTemplate 渲染——与桌面共用同一套页面。
    /// 维护约定：后续改动 MainWindow.axaml 的壳结构时须同步本方法。
    /// </summary>
    private static Control CreateAndroidMainView(MainViewModel mainViewModel)
    {
        // 与 MainWindow.axaml 四枚 PathIcon 逐字一致（保持视觉同一；键 ↔ MainViewModel.*PageKey）。
        ...
        navPanel.Children.Add(new TextBlock
        {
            ...
            Foreground = new SolidColorBrush(Color.Parse("#0067A0")),
            [DockPanel.DockProperty] = Dock.Top,
        });

        var highlight = new PageKeyToBrushConverter();
        var navButtons = new StackPanel { Spacing = 4, Margin = new Thickness(10, 0) };
        foreach (var page in mainViewModel.Pages)
        {
            var button = new Button
            {
                ...
                Content = new StackPanel
                {
                    ...
                    Children =
                    {
                        new PathIcon
                        {
                            Data = StreamGeometry.Parse(iconData[page.Key]),
                            Width = 18,
                            Height = 18,
                        },
                        new TextBlock { Text = page.Title, FontSize = 15 },
                    },
                },
            };

            // 与 MainWindow.axaml 同款“当前页高亮”：
            // Background ← PageKeyToBrushConverter(CurrentPageKey, key)。
            button.Bind(
                Button.BackgroundProperty,
                new Binding(nameof(MainViewModel.CurrentPageKey))
                {
                    Converter = highlight,
                    ConverterParameter = page.Key,
                });
        ...
        // 页面宿主：Content ↔ MainViewModel.CurrentPage；VM→View 模板在 App.axaml DataTemplates（桌面共用）。
